...
def translate_epub_file(
  ...
  try:
    ...
    _translate_folder(handler, unzip_path, book_title)
...
def _translate_folder(handler: EpubHandler, path: str, book_title: Optional[str]):
  ...
  _translate_spines(epub_content, path, handler)
...
def _translate_spines(epub_content: EpubContent, path: str,  handler: EpubHandler):
