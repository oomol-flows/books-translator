...
def main(inputs, context):
  if inputs["adapter"] == "google":
    adapter=Adapter.Google
  elif inputs["adapter"] == "open_ai":
  ...
  translator = Translator(
    ...
    source_language_code=inputs["source"],
    target_language_code=inputs["target"],
    max_paragraph_characters=inputs.get("max_paragraph_characters", 800),
  ...
  try:
    ...
    _translate_folder(inputs, unzip_path, translator)
    ...
    context.output(base64_str, "bin", True)

  finally:
    shutil.rmtree(unzip_path)

def _translate_folder(inputs, path: str, translator):
